...
export default function DashboardPage() {
  ...
  const handleAddTodo = async (e: React.FormEvent) => {
    ...
    try {
      const newTodo = await createTodo({
        ...
        status: 'pending',  // Changed from completed: false to status: 'pending'
      });
  ...
  const handleUpdateTodo = async (e: React.FormEvent) => {
    ...
    try {
      const updated = await updateTodo(editingTodo.id, {
        title: formData.title,
        description: formData.description,
        status: editingTodo.status,  // Changed from completed to status
      });
  ...
  const handleToggleTodo = async (id: string) => {  // Changed from number to string
    const todo = todos.find((t) => t.id === id);
    if (!todo) return;

    try {
      const updated = await toggleTodoComplete(id, todo.status !== 'completed'); // Changed to use status field
      setTodos(todos.map((t) => (t.id === id ? updated : t)));

      if (updated.status === 'completed') {  // Changed to use status field
        addToast({
          type: 'success',
  ...
  const handleDeleteTodo = async (id: string) => {  // Changed from number to string
    try {
      await deleteTodo(id);
