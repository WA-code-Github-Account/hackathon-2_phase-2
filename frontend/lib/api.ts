...
class ApiClient {
  ...
  async createTodo(todoData: CreateTodoRequest): Promise<Todo> {
    // Map frontend status to API status field
    const mappedData = {
      ...todoData,
  ...
  async toggleTodoComplete(id: string, completed: boolean): Promise<Todo> {
    // For toggle, we just call the complete endpoint which toggles the status
    // The backend handles the toggle logic, so we don't need to send any data
  ...
  private mapTaskToTodo(task: any): Todo {
    return {
      ...
      status: task.status || (task.completed ? 'completed' : 'pending'), // Map completed boolean to status string
