...
export interface TodoCardProps {
  todo: Todo;
  onToggle: (id: string) => void;  // Changed from number to string to match UUID
  onDelete: (id: string) => void;  // Changed from number to string to match UUID
  ...
  return (
    <div
      className={cn(
        ...
        todo.status === 'completed' && 'opacity-60 bg-muted/20',
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="flex items-start justify-between gap-4 mb-4">
          <div className="flex items-start gap-4 flex-1">
            ...
            >
              {todo.status === 'completed' && <CheckCircle2 className="w-4 h-4" />}
            </button>
